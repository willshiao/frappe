function [c,time] = efficient_corcondia4(X,Fac,sparse_flag)
...
try
    if(sparse_flag)
        ...
        [Ud Sd Vd] = svds(D,F);%disp('Calculated SVD of C');
    else
        ...
        [Ud Sd Vd] = svd(D,'econ');%disp('Calculated SVD of D');    
    end
catch exception
...
part1 = kron_mat_vec({Ua' Ub' Uc' Ud'},X);
part2 = kron_mat_vec({pinv(Sa) pinv(Sb) pinv(Sc) pinv(Sd)},part1);
G = kron_mat_vec({Va Vb Vc Vd},part2);%disp('Computed G');

T = sptensor([F F F F]);
for i = 1:F; T(i,i,i,i) =1; end

c = 100* (1 - sum(sum(sum(sum(double(G-T).^2))))/F);
time = toc;
end
